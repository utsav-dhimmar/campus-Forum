...
import { IMyAnswerResponse } from "@repo/shared";
...
const signUp = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res.status(201).json(new ApiResponse(201, userObj, "User create successfully"));
});
...
const newRefreshToken = asyncHandler(async (req: Request, res: Response) => {
  try {
    const incommingRefreshToken =
      req.cookies?.refreshToken || req.headers.authorization?.replace("Bearer", "").trim();

    if (!incommingRefreshToken) {
      throw new ApiError(401, "Unauthorized access , refresh token is missing");
    }

    const userInfo = jwt.verify(incommingRefreshToken, parsedEnv.REFRESH_TOKEN) as { _id: string };
...
const getMyAnswers = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!answers || answers.length === 0) {
    return res.status(200).json(new ApiResponse(200, [], "No answers found for this user"));
  }

  const formattedAnswers: IMyAnswerResponse[] = answers.map((answer) => ({
    _id: answer._id.toString(),
    body: answer.content,
    post: {
      _id: (answer.postId as any)._id.toString(),
    },
